  ...
  const setReorder = (orderingdata) => {
    ...
    if (orderingdata.length > 0) {
      const reorderedArray = orderingdata
        .map((orderItem) =>
        allProcess.find(
            (mainItem) => mainItem.id === orderItem.process_id
          )
        )
        .filter((item) => item);
                ...
                <div className="section-title mb-40 left-border">
                  <h2
                    ...
                    dangerouslySetInnerHTML={{
                      __html: section_data_4?.section_title,
                    }}
                  ></h2>
                  <div
                    dangerouslySetInnerHTML={{
                      __html: section_data_4?.section_content,
                    }}
                  ></div>
                </div>
              </div>
            ...
            <div className="col-lg-8 col-md-12">
              {allProcess &&
                ...
                allProcess.map((item,index) => {
                  return (
                    <div key={item.id} className="Process-list-col wow fadeInUp" 
                    data-wow-duration="1500ms"
